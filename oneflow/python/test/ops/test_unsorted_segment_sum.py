...
def _gen_data(np_type, out_shape, axis, segment_ids_shape):
    data_shape = out_shape[0:axis] + segment_ids_shape + out_shape[axis + 1 :]
    return np.random.uniform(-1, 1, size=data_shape).astype(np_type)
...
def _run_test(test_case, device, data_type, out_shape, axis, segment_ids_shape):
    ...
    data = _gen_data(np_type, out_shape, axis, segment_ids_shape)
